...
### ADR-006: Parent-forwards-signals pattern for webhook delivery to child workflows
**Date:** 2026-03-21
**Status:** accepted
**Context:** GitHub webhook signals (pr_review, check_complete, pr_closed, merge_queue_update) are dispatched to the parent orchestrator's workflow ID, but the pr_tracking child workflow needs them to update its review/check state in real time instead of polling. The child's workflow ID includes `phaseIteration` which changes across feedback loops, making it impractical for the webhook dispatcher to compute.
**Decision:** The parent orchestrator's signal handlers forward payloads to the active pr_tracking child using `getExternalWorkflowHandle(childId).signal()`, guarded by `currentPhase === "pr_tracking"`. Forwarding is wrapped in try/catch to handle race conditions where the child doesn't exist yet or has completed. Only the 4 GitHub lifecycle signals are forwarded; kill and cost override remain parent-scoped.
**Consequences:** Signal delivery adds Temporal commands to the parent's event history (one per forwarded signal). The webhook dispatcher remains simple (always targets the parent). New child-targeted signals must be added to both the parent's forwarding handlers and the child's signal registrations.
